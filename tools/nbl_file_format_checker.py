        ...
        try:
            ...
            with open(file_path, "rb") as f:
                ...
                (
                    ...
                    bbox_max_z,
                    reserved,
                ) = struct.unpack("<8sHHIHH3f3f4s", header_data)
                ...
                self.log(
                    f"Header 信息: FPS={target_fps}, 总帧数={total_frames}, 贴图数={tex_count}"
                )
                ...
                for i in range(tex_count):
                    ...
                    try:
                        ...
                        return (
                            False,
                            f"纹理路径不是有效的 UTF-8 编码 (Texture #{i})",
                            self.metadata,
                        )
                ...
                for i in range(total_frames):
                    ...
                    if chunk_offset + chunk_size > file_size:
                        return (
                            False,
                            f"帧 {i} 的数据块越界 (Offset {chunk_offset} + Size {chunk_size} > FileSize)",
                            self.metadata,
                        )
                ...
                if kf_count > total_frames:
                    self.log(
                        f"[Warning] 关键帧数量 ({kf_count}) 大于总帧数 ({total_frames})，这很奇怪。"
                    )
                ...
                for i, (offset, size) in enumerate(frame_indices):
                    ...
                    try:
                        ...
                        return (
                            False,
                            f"帧 {i} Zstd 解压失败: {str(e)} (可能是数据损坏或非独立压缩)",
                            self.metadata,
                        )
                    ...
                    if frame_type == 0:  # I-Frame
                        ...
                        return (
                            False,
                            f"帧 {i} 未知的 FrameType: {frame_type}",
                            self.metadata,
                        )
                    ...
                    if actual_payload_size != expected_payload_size:
                        return (
                            False,
                            (
                                f"帧 {i} 数据长度校验失败 (Type {frame_type}, N={particle_count})。\n"
...
class ValidationWorker(QObject):
    ...
    file_processed = pyqtSignal(
        str, bool, str, str, dict
    )  # filepath, is_valid, status_text, log_text, metadata
...
class NBLCheckerApp(QMainWindow):
    ...
    def add_files(self):
        files, _ = QFileDialog.getOpenFileNames(
            self, "选择 NBL 文件", "", "NebulaFX Files (*.nbl);;All Files (*)"
        )
        if files:
            for f in files:
    ...
    def show_log(self, row, col):
        ...
        if log:
            ...
            if metadata:
                ...
                display_text += (
                    f"总帧数 (Total Frames): {metadata.get('total_frames')}\n"
                )
                ...
                attr_list = []
                if attrs & 0x01:
                    attr_list.append("Alpha")
                if attrs & 0x02:
                    attr_list.append("Size")
